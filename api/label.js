...
function unwrapOrder(payload) {
  let o = payload;
  for (let i = 0; i < 5; i++) {
    if (!o || typeof o !== "object" || Array.isArray(o)) break;
    if (o.order && typeof o.order === "object") { o = o.order; continue; }
    if (o.data && typeof o.data === "object" && !Array.isArray(o.data)) {
      const d = o.data;
      if (d.order && typeof d.order === "object") { o = d.order; continue; }
      if (d.id || d.short_id || d.cart_items || d.items || d.products || d.order_items) { o = d; continue; }
    }
    break;
  }
  return o || {};
...
function findItemsDeep(root) {
  const preferred = ["cart_items", "items", "order_items", "products", "line_items", "order_products", "orderProducts", "details"];
  const seen = new Set();
...
function parseBodyData(o) {
  return normalizeItems(findItemsDeep(o));
}
...
.label{width:100%;height:100%;border:1px solid #111;border-radius:4mm;padding:2.7mm 2.8mm;display:grid;grid-template-rows:23mm 18mm 30mm minmax(20mm,1fr) 11mm 18mm 15mm;overflow:hidden}
...
.productsArea{min-height:20mm;height:100%;overflow:hidden;display:grid;grid-template-rows:auto minmax(0,1fr)}.productsTableWrap{min-height:0;height:100%;overflow:hidden;position:relative}.productsTableWrap .products{transform-origin:top left;will-change:transform;display:table}
...
.actions{position:fixed;left:14px;top:14px;z-index:60;display:flex;gap:7px}.actions button{border:0;border-radius:9px;padding:9px 13px;cursor:pointer;font-family:inherit;font-weight:700;box-shadow:0 2px 8px #0002}.edit{background:#07883f;color:#fff}.print{background:#111;color:#fff}
...
/* V5.10 side-by-side editor + live preview */
body{min-height:100vh;background:#f3f5f4;display:grid;grid-template-columns:minmax(360px,1fr) 112mm;grid-template-rows:1fr;grid-template-areas:"editor preview";gap:18px;padding:18px;align-items:start;direction:ltr}
.sheet{grid-area:preview;margin:0;width:100mm;height:150mm;justify-self:center;align-self:start;box-shadow:0 8px 28px #0002}
.modal{grid-area:editor;position:static;inset:auto;display:block;background:transparent;z-index:1;align-self:start;justify-self:stretch;min-width:0}
.modal.open{display:block}
.panel{width:100%;max-height:calc(100vh - 36px);overflow:auto;border-radius:16px}
.actions{position:fixed}
/* V5.10 editor layout: match the live preview and keep the product list visible. */
.modal{backdrop-filter:blur(3px)}
.panel{width:min(920px,96vw);background:#f7f8f8;border:1px solid #d9dedb;box-shadow:0 18px 50px #0003;padding:0;overflow:auto}
.panel h3{margin:0;padding:14px 18px;background:#075f31;color:#fff;font-size:18px;position:sticky;top:0;z-index:2}
.panel .field{margin:8px 0}.panel .field label{color:#075f31;font-size:11px}.panel .field input,.panel .field textarea{background:#fff;border-color:#d7d7d7;padding:9px 10px}
.panel .row{gap:10px}.panel .productsEdit{background:#fff;border-radius:10px;overflow:hidden}.panel .productsEdit th{background:#f2f5f3;color:#075f31}.panel .productsEdit input{border-color:#ddd}
.panel .buttons{position:sticky;bottom:0;background:#f7f8f8;padding:12px 0;margin-top:12px}.panel .save{background:#07883f}.panel .cancel{background:#e8e8e8}
@media(max-width:850px){body{display:flex;flex-direction:column;padding:10px;gap:12px}.sheet{order:1}.modal{order:2;width:100%}.actions{position:static;order:0;align-self:flex-start}.panel{max-height:none}.sheet{margin:0 auto}}
@media(max-width:650px){.panel .row{grid-template-columns:1fr}.productsEdit{font-size:9px}}

@media print{body{display:block;background:#fff;padding:0}.actions,.modal{display:none!important}.sheet{margin:0;width:100mm;height:150mm;box-shadow:none}.label{break-inside:avoid}}
</style></head><body>
...
<div class="field"><label id="productsEditTitle">المنتجات</label><table class="productsEdit"><thead><tr><th>المنتج</th><th>المقاس/اللون</th><th>الكمية</th><th>سعر الوحدة</th></tr></thead><tbody id="editRows"></tbody></table></div>
...
function fitProductTable(){
 ...
 const available=Math.max(1, wrap.getBoundingClientRect().height);
 const natural=Math.max(1, table.scrollHeight || table.getBoundingClientRect().height);
 if(natural<=available){ return; }
 const scale=Math.max(0.42, Math.min(1, available/natural));
 table.style.transformOrigin='top left';
 table.style.transform="scale("+scale+")";
 table.style.width=(100/scale)+"%";
}
function fitEditorProductTable(){
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    fitProductTable();
    setTimeout(fitProductTable,60);
  }));
}
function render(){
  const d=getData();
...
function openEdit(){
 ...
 document.getElementById("productsEditTitle").textContent="المنتجات ("+(d.items||[]).length+")"; editRows.innerHTML=(d.items||[]).map((x,i)=>'<tr><td><input data-i="'+i+'" data-f="name" value="'+escClient(x.name)+'"></td><td><input data-i="'+i+'" data-f="variant" value="'+escClient(x.variant||"")+'"></td><td><input data-i="'+i+'" data-f="quantity" type="number" min="1" step="1" value="'+Number(x.quantity||1)+'"></td><td><input data-i="'+i+'" data-f="price" type="number" min="0" step=".01" value="'+Number(x.price||0)+'"></td></tr>').join("");
 fitEditorProductTable();
}
...
function closeEdit(){toast("التعديلات الحالية محفوظة محليًا عند الضغط على حفظ")}
...
render();
fitEditorProductTable();
window.addEventListener("resize", fitEditorProductTable);
</script></body></html>`);
  } catch(e) {
