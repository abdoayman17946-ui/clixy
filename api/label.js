...
.productSection{margin-top:1.7mm;border:1px solid #888;border-radius:2.3mm;display:grid;grid-template-columns:30mm 1fr;gap:2mm;align-items:center;padding:1.5mm;min-height:31mm;overflow:hidden}.productQrWrap{display:flex;align-items:center;justify-content:center}.productQr{width:26mm;height:26mm;border:1px solid #bd8a1d;border-radius:2mm;padding:1mm;display:block}.productDesc{min-width:0}.descLabel{font-size:6.5px;color:#bd8a1d;font-weight:800;margin-top:1mm}.descText{font-size:7.4px;font-weight:700;line-height:1.35;white-space:pre-wrap;word-break:break-word;min-height:10mm;max-height:17mm;overflow:hidden}
...
<div><div class="k">العنوان</div><div class="v" id="address"></div><div class="locationBox"><div class="k">اللوكيشن</div><img class="locationQr" id="locationQr" style="display:none" alt="Location QR"></div></div>
<div><div class="k">ملاحظات</div><div class="v" id="miniNotes"></div></div>
</div>
<div class="productSection"><div class="productQrWrap"><img id="productQr" class="productQr" alt="QR المنتجات"></div><div class="productDesc"><div class="productsTitle">المنتجات</div><div class="descLabel">وصف المنتج</div><div class="descText" id="productDescription">-</div></div></div>
...
<div class="row"><div class="field"><label>الديبوزيت المدفوع</label><input id="eDeposit" type="number" min="0" step=".01"></div><div class="field"><label>إجمالي الطلب (يتحدث تلقائيًا)</label><input id="eTotal" type="number" min="0" step=".01" readonly></div></div>
...
<div class="field"><label>وصف المنتج</label><textarea id="eProductDescription" rows="3" placeholder="اكتب وصف المنتج الذي تريد ظهوره على البوليصة"></textarea><div style="font-size:10px;color:#777;margin-top:4px">سيظهر الوصف كنص على البوليصة، بينما QR المنتجات يحتوي على المنتجات والكميات فقط.</div></div>
...
function render(){
  ...
  const productText=items.map(x=>`${x.name||"منتج"}${x.variant?" - "+x.variant:""} × ${Number(x.quantity||1)}`).join("\n");
  const productQr=document.getElementById("productQr");
  if(productText){productQr.src="https://api.qrserver.com/v1/create-qr-code/?size=240x240&margin=8&data="+encodeURIComponent(productText);productQr.style.display="block";}else{productQr.removeAttribute("src");productQr.style.display="none";}
  document.getElementById("productDescription").textContent=d.product_description||"-";
}
...
function openEdit(){
 ...
 const items=Array.isArray(d.items)?d.items:[];
 const subtotal=items.reduce((s,x)=>s+(Number(x.price)||0)*(Number(x.quantity)||0),0);
 eDeposit.value=Number(d.deposit||0);
 eShipping.value=Number(d.shipping||0);
 eTotal.value=(subtotal+Number(d.shipping||0)).toFixed(2);
 updateCodField();
 eName.value=d.full_name||"";ePhone2.value=d.phone2||"";eAddress.value=d.address||"";eLocation.value=d.location||"";eNotes.value=d.notes||"";eProductDescription.value=d.product_description||"";
 modal.classList.add("open");
}
function updateTotalsFromShipping(){
 const d=getData();
 const items=Array.isArray(d.items)?d.items:[];
 const subtotal=items.reduce((s,x)=>s+(Number(x.price)||0)*(Number(x.quantity)||0),0);
 const shipping=Math.max(0,Number(eShipping.value)||0);
 eTotal.value=(subtotal+shipping).toFixed(2);
 updateCodField();
}
...
eShipping.addEventListener("input",updateTotalsFromShipping);
function saveEdit(){
 try {
  const d=getData();
  const items=Array.isArray(d.items)?d.items:[];
  ...
  d.product_description=document.getElementById("eProductDescription").value.trim();
