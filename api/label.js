...
.header{display:grid;grid-template-columns:1fr 1fr 1fr;align-items:center;direction:ltr;gap:2mm;padding-bottom:2.1mm;border-bottom:1.4px solid #bd8a1d}
...
function fitProductTable(){
 ...
 table.style.transform='scale(1)'; table.style.width='100%';
 const available=wrap.clientHeight; const natural=table.scrollHeight;
 if(available>0 && natural>available){
   const scale=Math.max(0.28, Math.min(1, available/natural));
   table.style.transformOrigin='top left'; table.style.transform=`scale(${scale})`; table.style.width=`${100/scale}%`;
 }
}
