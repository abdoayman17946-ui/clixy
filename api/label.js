...
.info{border:1px solid #888;border-radius:2.3mm;display:grid;grid-template-columns:1fr 1.45fr 1fr;min-height:25mm;overflow:hidden}.info>div{padding:1.7mm 2mm;border-left:1px dashed #aaa}.info>div:last-child{border-left:0}.info .k{font-size:7px;color:#b98212;margin-bottom:.8mm}.info .v{font-size:8.4px;font-weight:700;line-height:1.35}.phone{direction:ltr;text-align:right;font-size:8.3px;font-weight:700;margin-top:.8mm}
...
<header class="header"><div><img class="logo" src="${logoUrl}" alt="CLIXY"></div><div class="headCenter"><div class="small">رقم البوليصة</div><div class="orderNo"><span class="hash">#</span>${esc(orderNo)}</div></div><div class="qrbox"><img class="qr" src="${qrUrl}" alt="QR"><div class="qrurl"></div></div></header>
<div class="barcodeWrap"><div class="barcodeTitle">رقم البوليصة</div>${code128Svg(barcodeValue).replace('<svg ', '<svg class="barcode" ')}<div class="barcodeText">${esc(barcodeValue)}</div></div>
<div class="info">
<div><div class="k">اسم العميل</div><div class="v" id="fullName"></div><div class="k" style="margin-top:1.7mm">رقم الهاتف الأساسي</div><div class="phone" id="phone"></div></div>
<div><div class="k">العنوان</div><div class="v" id="address"></div></div>
<div><div class="k">رقم إضافي (اختياري)</div><div class="phone" id="phone2"></div><div class="k" style="margin-top:3mm">ملاحظات</div><div class="v" id="miniNotes"></div></div>
</div>
...
<div class="footer"><div class="footerBox"><div class="k">ملاحظات الطلب</div><div class="v" id="notes">-</div></div><div class="footerBox"><div class="k">رقم الطلب</div><div class="v">ORDER-${esc(orderNo)}</div></div><div class="footerBox"><div class="k">خدمة العملاء</div><div class="v">01033674242</div></div></div>
</section></div>
...
<div class="row"><div class="field"><label>المبلغ المطلوب تحصيله</label><input id="eCod" type="number" min="0" step=".01"></div><div class="field"><label>الشحن</label><input id="eShipping" type="number" min="0" step=".01"></div></div>
...
function render(){
  ...
  const cod=d.codManual?Math.max(0,Number(d.cod)||0):Math.max(0,total-dep);
  ...
  document.getElementById("notes").textContent=d.notes||"شكراً لشرائك من CLIXY ❤️";
...
function openEdit(){
 const d=getData();
 eDeposit.value=d.deposit||0;eTotal.value=Number(d.total||0);eCod.value=d.codManual?Number(d.cod||0):Math.max(0,Number(d.total||0)-Number(d.deposit||0));eShipping.value=Number(d.shipping||0);
 eName.value=d.full_name||"";ePhone2.value=d.phone2||"";eAddress.value=d.address||"";eNotes.value=d.notes||"";
...
function saveEdit(){
 const d=getData();
 d.deposit=Math.max(0,Number(eDeposit.value)||0);
 d.total=Math.max(0,Number(eTotal.value)||0);
 d.shipping=Math.max(0,Number(eShipping.value)||0);
 d.full_name=eName.value.trim();d.phone2=ePhone2.value.trim();d.address=eAddress.value.trim();d.notes=eNotes.value.trim();
 d.cod=Math.max(0,Number(eCod.value)||0);
 d.codManual=Math.abs(d.cod-(d.total-d.deposit))>0.009;
 d.items=(d.items||[]).map((x,i)=>{const get=f=>document.querySelector('#editRows [data-i="'+i+'"][data-f="'+f+'"]');return {...x,name:get("name").value.trim(),variant:get("variant").value.trim(),quantity:Math.max(1,Number(get("quantity").value)||1),price:Math.max(0,Number(get("price").value)||0)}}).filter(x=>x.name);
 localStorage.setItem(ORDER_KEY,JSON.stringify(d));render();
eDeposit.addEventListener("input",()=>{const t=Math.max(0,Number(eTotal.value)||0);const d=Math.max(0,Number(eDeposit.value)||0);eCod.value=Math.max(0,t-d).toFixed(2)});
eTotal.addEventListener("input",()=>{const t=Math.max(0,Number(eTotal.value)||0);const d=Math.max(0,Number(eDeposit.value)||0);eCod.value=Math.max(0,t-d).toFixed(2)});
closeEdit();
}
render();
</script></body></html>`);
